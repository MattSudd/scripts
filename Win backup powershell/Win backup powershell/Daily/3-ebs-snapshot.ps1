...
function snapshot_volumes {
	foreach($volume_id in $volume_list)	{
		...
		aws ec2 create-tags --region $region --resource $snapresult --tags Key="CreatedBy,Value=DailyAutomatedBackup"
		$global:log_message = $global:log_message + "Volume ID is $volume_id." + $nl
	}
}
...
function cleanup_snapshots {
	foreach($volume_id in $volume_list) {
		$snapshot_list = aws ec2 describe-snapshots --region $region --output=text --filters "Name=volume-id,Values=$volume_id" "Name=tag:CreatedBy,Values=DailyAutomatedBackup" --query Snapshots[].SnapshotId | %{$_.split("`t")}
...
log "Info"
write-host "Daily backup script complete. Results written to the Event Log (check under Applications, Event ID 1337)."
