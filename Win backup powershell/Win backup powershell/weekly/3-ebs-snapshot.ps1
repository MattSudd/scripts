...
# Write to console and Application event log (event ID: 1338)
function log ($type) {
	Write-Host $global:log_message
	Write-EventLog -LogName Application -Source "EBS-Snapshot" -EntryType $type -EventID 1338 -Message $global:log_message
}
...
function snapshot_volumes {
	foreach($volume_id in $volume_list)	{
		...
		aws ec2 create-tags --region $region --resource $snapresult --tags Key="CreatedBy,Value=WeeklyAutomatedBackup"
		$global:log_message = $global:log_message + "Volume ID is $volume_id." + $nl
	}
}
...
function cleanup_snapshots {
	foreach($volume_id in $volume_list) {
		$snapshot_list = aws ec2 describe-snapshots --region $region --output=text --filters "Name=volume-id,Values=$volume_id" "Name=tag:CreatedBy,Values=WeeklyAutomatedBackup" --query Snapshots[].SnapshotId | %{$_.split("`t")}
...
function log_context {
   $global:log_message = $global:log_message + "Daily Snapshot Job Complete" + $nl 
}
...
log "Info"
write-host "Weekly backup script complete. Results written to the Event Log (check under Applications, Event ID 1338)."
